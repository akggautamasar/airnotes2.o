...
import { Grid, List, RefreshCw, AlertCircle, Loader2, Sun, Moon } from 'lucide-react';
...
export default function LibraryView({ onSearch }) {
  ...
  const displayFiles = useMemo(() => {
    ...
    if (activeFolderId) {
      ...
      files = files
        .filter(f => progresses[f.id] && progresses[f.id].percent > 0 && progresses[f.id].percent < 100)
        .sort((a, b) => (progresses[b.id]?.updatedAt || 0) - (progresses[a.id]?.updatedAt || 0));
    ...
    if (activeSection !== 'recent' && activeSection !== 'continue') {
      files = [...files].sort((a, b) => (b.date || 0) - (a.date || 0));
    }

    return files;
  }, [state.files, state.activeSection, state.activeFolderId, state.fileAssignments,
      state.recentFiles, state.favorites, progresses, typeFilter]);
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="text-center max-w-sm">
          ...
          <button onClick={handleRefresh} className="btn-primary px-4 py-2 rounded-xl text-sm flex items-center gap-2 mx-auto">
